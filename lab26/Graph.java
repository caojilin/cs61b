...
public class Graph {
    ...
    public Graph prims(int start) {
        ...
        while (!pq.isEmpty()) {
            Edge e = pq.peek();
            pq.poll();
            if (visited.contains(e.getSource()) && visited.contains(e.getDest())) {
                continue;
            }
            visited.add(e.getSource());
            for (Edge edge : edges.get(e.getDest())) {
    ...
    public Graph kruskals() {
        // YOUR CODE HERE
        Graph mst = new Graph();
        WeightedQuickUnionUF unionUF = new WeightedQuickUnionUF(getAllVertices().size());
        ArrayList<Edge> ls = new ArrayList<>();
        for (Edge e : getAllEdges()) {
            ls.add(e);
        }
        ls.sort((o1, o2) -> (o1.getWeight() - o2.getWeight()));
        for (int i = 0; i < ls.size(); i++) {
            Edge temp = ls.get(i);
            if (unionUF.find(temp.getSource()) != unionUF.find(temp.getDest())) {
                unionUF.union(temp.getSource(), temp.getDest());
                mst.addEdge(temp);
            }
