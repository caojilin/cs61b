...
public class DLList<T extends Comparable<T>> {
    ...
    public DLList<T> mergeSort() {
        ...
        for (int i = 1; i <= mid; i++) {
            oneHalf.addLast(curr.item);
            curr = curr.next;
        }
        while (curr != sentinel) {
            otherHalf.addLast(curr.item);
            curr = curr.next;
        }
    ...
    public DLList<T> quicksort() {
        ...
        Node indicator = sentinel.next;
        while (indicator != sentinel) {
            T temp = indicator.item;
            if (indicator.item.compareTo(pivot) > 0) {
                largeElements.addLast(temp);
            } else if (indicator.item.compareTo(pivot) < 0) {
                smallElements.addLast(temp);
            } else {
                equalElements.addLast(temp);
            }
